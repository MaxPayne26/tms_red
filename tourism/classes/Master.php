...
Class Master extends DBConnection {
	...
	function save_tour(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `tourist_spot` set {$data} ";
			$save = $this->conn->query($sql);
			$id = $this->conn->insert_id;
		}else{
			$sql = "UPDATE `tourist_spot` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			if(isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0){
				...
				$this->conn->query(query: "UPDATE `tourist_spot` set {$data} where id = '{$id}' ");
				foreach($_FILES['img']['tmp_name'] as $k =>$v){
					move_uploaded_file( $_FILES['img']['tmp_name'][$k],base_app.'uploads/package_'.$id.'/'.$_FILES['img']['name'][$k]);
				}
			}
			...
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"New Tourist Spot successfully saved.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_tour(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `tourist_spot` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			if(is_dir(base_app.'uploads/package_'.$id)){
			...
			$this->settings->set_flashdata('success',"Tourist Spot successfully deleted.");
		}else{
			$resp['status'] = 'failed';
	...
	function book_tour(){
		// Check if the required data is set and sanitize it
		$user_id = $this->settings->userdata('id');
		
		// Check if the 'schedule' field is provided in the form submission
		if (!isset($_POST['schedule']) || empty($_POST['schedule'])) {
			return json_encode(['status' => 'failed', 'error' => 'Schedule date is required.']);
		}
	
		// Sanitize input data
		$schedule = $this->conn->real_escape_string($_POST['schedule']);
		$tourist_spot_id = isset($_POST['tourist_spot_id']) ? $this->conn->real_escape_string($_POST['tourist_spot_id']) : null;
		$package_id = isset($_POST['package_id']) ? $this->conn->real_escape_string($_POST['package_id']) : null;
		...
		$query = "INSERT INTO `book_list` (`user_id`, `schedule`, `tourist_spot_id`, `package_id`) 
				  VALUES (?, ?, ?, ?)";
	
		// Prepare and bind the query
		if ($stmt = $this->conn->prepare($query)) {
			// Bind parameters (s = string, i = integer)
			$stmt->bind_param("ssss", $user_id, $schedule, $tourist_spot_id, $package_id);
			...
			if ($stmt->execute()) {
				...
				$resp['status'] = 'failed';
				$resp['error'] = 'Failed to execute query: ' . $this->conn->error;
			}
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = 'Failed to prepare the query: ' . $this->conn->error;
		}
...
switch ($action) {
	...
	break;
	case 'save_tour':
		echo $Master->save_tour();
	break;
	case 'delete_package':
		echo $Master->delete_package();
	break;
	case 'delete_tour':
		echo $Master->delete_tour();
	break;
	case 'delete_p_img':
